...
class AchievmentRepository(DatabaseHelper):
    ...
    def create_new_achievments(
        self, achievementName, achievementPoints, condition_type, value
    ):
        self.cursor.execute(
            """INSERT INTO Achievement (achievementName, achievementPoints, condition_type, value) VALUES (?, ?, ?,?) """,
            (achievementName, achievementPoints, condition_type, value),
        )
        self.con.commit()

    def get_requierments(self, achievementID):
        requierments = self.get_value_from_table(
            "Achievement", "condition_type, value", "achievementID", achievementID
        )
